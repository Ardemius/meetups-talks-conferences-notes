...
* 1er client est également le fondateur d'Indexima
	** "Benoît", qui est maintenant un CxO d'Indexima
...
image::20201103_startech_indexima_04.JPG[]
	...
	* *Hyper Index* : ces derniers sont créés non pas par des humains *mais des algos*
		...
		** on a l'impression qu'on fait des requêtes sur des milliards de lignes, alors qu'en fait on requêtes un subset particulièrement adapté à nos requêtes
...
image:20201103_startech_indexima_06.JPG[]
image:20201103_startech_indexima_07.JPG[]
image:20201103_startech_indexima_08.JPG[]
image:20201103_startech_indexima_09.JPG[]

=== Demo

* itinéraires des taxies de New York : ~5 milliards de lignes
...
* Indexima va venir "attraper" les requêtes de Snowflake
	** après une phase de "chauffe" ?
		*** pour comprendre le pattern d'usage de mon utilisation de Tableau

* Indexima utilise le CDC (Change Data Capture) de Snowflake pour être tenu au courant des mises à jour de la Data

[IMPORTANT]
====
Indexima ressemble à la solution de création de cubes custom en mémoire que nous avions mis en place à la SGCIB.

	* on va donc créer à la volée des cubes optimisé à nos besoins
====

La "magie" d'Indexima vient de ses algos permettant de créer les hyper indexes / datamart adaptés à nos besoins. 

revue de l'optim possible ?
...
* Esteban nous confirme que le principal use case d'Indexima est *l'optimisation de requêtes BI*
	** un use case secondaire est qu'Indexima peut également servir de couche d'abstraction pour accéder aux systèmes de persistance sous-jacent.
