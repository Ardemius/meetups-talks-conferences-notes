...
For Grace, *Reactive architecture* is the future for Application architecture.
...
Differences between Reactive programming vs Reactive systems?

Grace fait l'analogie avec une équipe de foot composée uniquement de Messi. +
Individuellement, ils sont très forts, mais ne savent pas jouer ensemble. +
-> Pour devenir un *Reactive system*, il faut rajouter cette cohésion manquante.

Donc, des applications individuellement réactives ne s'assemblent pas naturellement en 1 système réactif.

Pour rajouter cette cohésion manquante et obtenir un système réactif, il faut ajouter :

* DDD
* asynchronous communication
* Event-sourcing / CQRS
* High availability / Eventual Consistency (CA of the CAP theorem)
* sharding
* back Pressure
* bulkheading : "the publisher determines how many bees should go to the flower" +
La fleur étant ici la ressource dont il faut protéger l'usage
* Circuit breaking

.Exemples de frameworks réactifs
image:tuesday_53.jpg[]

TIP: Grace conseille le cours sur la *programmation réactive par Lightbend sur Cousera*

.Resources
image:tuesday_54.jpg[]

*Avis* : je ne suis pas totalement d'accord avec l'analyse de Grace, que j'avais trouvée trop "bunch of tools and features oriented" ("il faut avoir ça ça ça et ça...", mais il y avait de bonnes choses dans sa présentation basée sur les abeilles 😉

== Synthèse, buzz words & Co

Beaucoup de conf redonnent les bases sur les microservices : que sont-ils, quels sont leurs avantages, leurs faiblesses, etc. +
-> Comme quoi c'est encore nécessaire, nous ne sommes pas arrivés à un point ou tout le monde sait vraiment ce que sont des microservices.
...
* *Serverless* de plus en plus
	** En particulier *Knative* revient très souvent (qui utilise *Istio* si je ne dis pas de bêtise)
.Knative
[NOTE]
====
Pour rappel, https://cloud.google.com/knative[Knative] (Google) "offre un ensemble essentiel de composants pour créer et exécuter des applications sans serveur sur Kubernetes".
====
